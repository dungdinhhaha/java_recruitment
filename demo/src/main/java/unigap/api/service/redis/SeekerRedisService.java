...
public class SeekerRedisService implements IRedisService{
    ...
    public List<?> getAll(String keyword, String method, PageRequest pageRequest) throws JsonProcessingException {
        ...
        try {
            if (json != null) {
                productResponses = redisObjectMapper.readValue(json, new TypeReference<List<SeekerOut>>() {});
            }
    ...
    @Override
    public void saveAll(List<?> SeekerOut, String keyword, String method, PageRequest pageRequest) throws JsonProcessingException {
        String key = this.getKeyFrom(keyword, method, pageRequest);
        String json = redisObjectMapper.writeValueAsString(SeekerOut);
